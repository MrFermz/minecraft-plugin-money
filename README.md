...
money register `MenuItem` (key `money.top.visible`, default เปิด) เข้า `MenuRegistry` ของ core ตอน `onEnable` — ผู้เล่นปิดได้ผ่าน `/menu` ถ้าปิด `/money top` จะกรองชื่อออกก่อนจัดอันดับ (อ่านค่าผ่าน `PlayerPreferenceService` แบบ realtime, ได้ทั้งคน online/offline เพราะ core cache ไว้) — เป็น optional: ถ้า core ไม่ register service ไว้ money ถือว่าทุกคนโชว์
...
- แต่ละแถวมี `id` (PK gen ด้วย UUID) + เก็บ: `created_at` (**date จริง** — `TIMESTAMP`/`DATETIME` ไม่ใช่ epoch number), `type` (`PAY/TRANSFER/GIVE/TAKE/SET/RESET/DEPOSIT/WITHDRAW`), `created_by` (คนสั่ง — null = console/system), `from_uuid`/`to_uuid`, `amount`, `from_balance`/`to_balance`, `reason` — player ref **เก็บเป็น UUID ล้วน** (resolve ชื่อตอนแสดงผล ไม่เก็บชื่อค้างให้เพี้ยน)
